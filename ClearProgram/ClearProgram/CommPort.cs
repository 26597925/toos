...
namespace ClearProgram
{
    ...
    public class CommPort
    {
        ...
        public void Write(byte[] WriteBytes, int BytesWritten, int count)
        {
            if (hComm != INVALID_HANDLE_VALUE)
            {
                OVERLAPPED ovlCommPort = new OVERLAPPED();
                WriteFile(hComm, WriteBytes, count, ref BytesWritten, ref ovlCommPort);
            }
            else
            {
                throw (new ApplicationException("串口未打开！"));
        ...
        public void Write(string WriteString, Encoding CoderType)
        {
            byte[] data = CoderType.GetBytes(WriteString);
            Write(data, 0, data.Length);
        }

        public void Write(string WriteString)
        {
            byte[] data = Encoding.ASCII.GetBytes(WriteString);
            Write(data, 0, data.Length);
        }
    }
